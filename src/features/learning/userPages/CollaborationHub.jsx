...
import useUserDiscussion from "../userHooks/useUserDiscussion";

export default function CollaborationHub() {
  const courseId = "123"; // dynamic later (testing)
  const currentUserId = "me"; // replace with auth user

  const { messages, sendMessages } = useUserDiscussion(courseId);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="sticky top-16 z-10 px-5 md:px-10 pt-5 md:pt-10 pb-4 md:pb-6 bg-[#1E3A5F]">
        <h1 className="font-bold mb-0 text-white text-[24px] md:text-[36px]">
          Collaboration Hub
        </h1>
      </div>
      ...
      <div className="px-4 md:px-6 py-6 mb-10">
        <div className="flex flex-col md:flex-row rounded md:rounded-2xl md:py-6 md:px-4 px-4 py-2 border-[#D97706] md:justify-between border">
          <div className="mb-2 md:flex md:gap-4 md:items-center">
            ...
            <p className="text-[#1F2937] text-[13px] md:text-[18px] leading-5">
              Reminder: Week 3 project submission deadline is friday, March
              28th, before 11:59pm
            </p>
          </div>
          <p className="text-[#7C3AED] text-[13px] md:text-[14px] font-semibold">
            View all
          </p>
        </div>

        <div className="mb-2 mt-4 md:mt-8">
          <p className="text-[#1F2937] text-[18px] md:text-[20px] font-semibold">
            Your Tools
          </p>
          <p className="text-[13px] md:text-[16px] text-[#6B7280]">
            Access your collaboration tools
          </p>
        </div>
          ...
          >
            <div className="flex items-center mb-1 gap-1 md:gap-2 shadow-lg  ">
              <img
                ...
                alt=""
              />
              <p className="text-[#1F2937] text-[14px] md:text-[20px] font-semibold">
                Zoom
              </p>
            </div>
        ...
        <div className="flex flex-col-reverse md:justify-between md:gap-8 md:items-start md:flex-row">
            ...
            {items.map((i) => (
              <div key={i.name}>
                <div className="flex gap-2">
                  <img
                    src={i.img}
                    className="w-10 h-10 md:w-12 md:h-12 border-2 border-[#0029F5] rounded-full object-cover"
                    alt="image"
                  />
                  <div>
                    <p className="text-[#1F2937] text-[13px] md:text-[14px] font-semibold">
                      {i.name}
                    </p>
                    <p className="text-[12px] text-[#6B7280] md:text-[14px]">
                      {i.role}
                    </p>
                  </div>
                </div>
          ...
          <div className="border mb-5 border-[#E5E7EB] md:justify-between md:gap-4 md:bg-white md:border-0 bg-[#F9FAFB] md:rounded md:w-1/2 flex justify-between items-center px-4 py-2">
            <input
              ...
              className="text-[#6B7280] md:focus:border-[#7C3AED] md:w-full md:p-2 md:border md:border-[#E5E7EB] outline-0 text-[14px] bg-[#F9FAFB]"
            />
            <div
              onClick={handleSend}
              className="md:p-2 rounded md:border md:border-[#E5E7EB] md:bg-[#F9FAFB]"
            >
