    ...
    <table className=" hidden w-full min-[1150px]:block ">
      <thead className="bg-[#F0F3FF1A]">
        <tr className="text-left">
          ...
          {tableHeaders.map((header) => (
            <th
              key={header}
              className=" text-[#6B7280] text-[14px] font-medium tracking-normal leading-5.25 py-4.5 px-6"
            >
              {header}
            </th>
          ))}
      ...
      <tbody>
        {currentUsers.map((user) => (
          <tr
            key={user.id}
            className="border-b border-[#CCC3D81A] hover:bg-gray-50 "
          >
            {/* Checkbox */}
            <td className="pl-20">
              <input
            ...
            <td className="py-4.5 px-6">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-[#001C3B] text-[14px] font-semibold tracking-normal leading-5">
                    {user.name}
                  </p>
                  <p className="text-[#455F87] text-[11px] tracking-[normal] leading-[100%]">
                    {user.email}
                  </p>
                </div>
              </div>
            ...
            <td className="py-4.5 px-6 tracking-normal leading-[100%]">
              {user.team ? (
                <span
                  className={` 
                                   ${
                                     user.team.toLowerCase() === "design"
                                       ? "bg-[#FDF2F8] text-[#BE185D]"
                                       : user.team.toLowerCase() ===
                                           "engineering"
                                         ? "bg-[#EEF2FF] text-[#0029F5]"
                                         : user.team.toLowerCase() ===
                                             "marketing"
                ...
                <span className=" py-1 px-2.5 rounded-full text-2.5 font-bold bg-[#E7EEFF] text-[#1E3A5F]">
                  Unassigned
                </span>
              )}
            </td>
